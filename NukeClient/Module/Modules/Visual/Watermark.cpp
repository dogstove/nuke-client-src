...
void Watermark::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (g_Data.canUseMoveKeys() && !clickGUI->hasOpenedGUI) {
		...
		float wit = DrawUtils::getTextWidth(&std::string("Nuke - V6 Beta"), 1.f);
		...
		DrawUtils::drawText(vec2_t(2, 2), &std::string("Nuke - V6 Beta"), MC_Color(interfaceColor), 1.f, 1.f, true);
		if (outlinec) {
