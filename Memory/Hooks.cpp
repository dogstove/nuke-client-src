...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			*/
			float wtextWid = DrawUtils::getTextWidth(&string("Night Client"), 2.f);
			...
			DrawUtils::drawText(vec2_t(wid.x - wtextWid - 5, wid.y - wid.y + 7), &string("Night Client"), MC_Color(interfaceColor), 2.f, 1.f, true);
